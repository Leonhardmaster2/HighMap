...
inline float helper_vflux(float f0,
                          float dh,
                          float dt,
                          float plength,
                          float gv,
                          ...
                          float power)
{
  ...
  float slope = max(0.f, dh) / plength;
  float tau = gv * slope;

  // f += dt * gv * slope;

  if (fabs(tau) > yield_stress)
  {
    //      f += dt * (tau - yield_stress) / viscosity;

    float drive = tau - yield_stress;
    float accel = pow(drive, power) / viscosity;
    f += dt * accel;
  }
...
void kernel
hydraulic_vpipes_viscous_flow_pass(read_only image2d_t  z,
                                   read_only image2d_t  fl,
                                   read_only image2d_t  fr,
                                   read_only image2d_t  ft,
                                   read_only image2d_t  fb,
                                   ...
                                   const float          flux_diffusion_strength)
{
  ...
  float fl0 = TGET(fl, i, j);
  float fr0 = TGET(fr, i, j);
  float ft0 = TGET(ft, i, j);
  float fb0 = TGET(fb, i, j);

  float fl_new =
      helper_vflux(fl0, dhl, dt, plength, gv, viscosity, yield_stress, power);
  float fr_new =
      helper_vflux(fr0, dhr, dt, plength, gv, viscosity, yield_stress, power);
  float ft_new =
      helper_vflux(ft0, dht, dt, plength, gv, viscosity, yield_stress, power);
  float fb_new =
      helper_vflux(fb0, dhb, dt, plength, gv, viscosity, yield_stress, power);

  /*   // helper macro */
  /*   /\* #define UPDATE_FLUX(f0, dh) \ *\/ */
  /*   /\*   ({ \ *\/ */
  /*   /\*     float slope = max(0.f, dh); \ *\/ */
  /*   /\*     float tau = gv * slope; \ *\/ */
  /*   /\*     float f = f0; \ *\/ */
  /*   /\*     if (tau > yield_stress) \ *\/ */
  /*   /\*     { \ *\/ */
  ...
  /* #define UPDATE_FLUX(f0, dh) (max(0.f, f0 + dt * gv * dh / plength)) */

  /*   float fl_new = UPDATE_FLUX(fl0, dhl); */
  /*   float fr_new = UPDATE_FLUX(fr0, dhr); */
  /*   float ft_new = UPDATE_FLUX(ft0, dht); */
  /*   float fb_new = UPDATE_FLUX(fb0, dhb); */

  /* #undef UPDATE_FLUX */

  /* float fl_new = max(0.f, fl0 + dt * gv * dhl / plength); */
  /* float fr_new = max(0.f, fr0 + dt * gv * dhr / plength); */
  /* float ft_new = max(0.f, ft0 + dt * gv * dht / plength); */
  /* float fb_new = max(0.f, fb0 + dt * gv * dhb / plength); */

  // optional flux diffusion (numerical smoothing)
  if (flux_diffusion)
  {
    float c0 = flux_diffusion_strength;
    float c1 = 1.f - 4.f * c0;
    float fsum = fl_new + fr_new + ft_new + fb_new;

    fl_new = c1 * fl_new + c0 * fsum;
    fr_new = c1 * fr_new + c0 * fsum;
    ft_new = c1 * ft_new + c0 * fsum;
    fb_new = c1 * fb_new + c0 * fsum;
  }

  // normalize
  float sum = fl_new + fr_new + ft_new + fb_new;
  float k = 0.f;
  if (sum > 1e-6f) k = TGET(d1, i, j) * plength * plength / (sum * dt);
  k = clamp(k, 0.f, 1.f);
