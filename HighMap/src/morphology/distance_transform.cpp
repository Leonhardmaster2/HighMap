                                      ...
                                      bool             return_squared_distance)
{
  ...
  // closest indices after row pass
  Mat<int> row_closest_i(array.shape);
  Mat<int> row_closest_j(array.shape);
  ...
  for (int i = 0; i < ni; i++)
  {
    ...
    for (int j = 1; j < nj; j++)
    {
      ...
      {
        g(i, j) = 1.f + g(i, j - 1);
        row_closest_i(i, j) = row_closest_i(i, j - 1);
        row_closest_j(i, j) = row_closest_j(i, j - 1);
      }
    }
