 ...
 * the positions and values of the points are randomly generated within a given
 * bounding box.
 *
 ...
 * @param default_value The default value assigned to each point. Defaults to 0
 *                      if not specified.
 */
 ...
 *
 * This constructor allows the creation of a cloud by providing separate lists
 * of `x` and `y` coordinates, along with a list of values associated with each
 * point.
 *
 * @param x A vector of `x` coordinates for the points.
 * @param y A vector of `y` coordinates for the points.
 ...
 *
 * @note The CSV file must be well-formed, with each line containing either 2 or
 * 3 comma-separated values. Lines with an unexpected number of values will
 ...
 *
 * @return Vec4<float> The bounding box of the cloud in the format `[xmin, xmax,
 *         ymin, ymax]`.
 */
 ...
 *
 * This function computes the center (or centroid) of a collection of points by
 * averaging their coordinates. It sums up all the point coordinates and then
 * divides the result by the total number of points to obtain the average
...
/**
 * @brief Computes the indices of the points that form the convex hull of a set
 * of points.
 *
 ...
 *
 * This method returns a vector containing the values associated with each point
 * in the cloud.
 *
 ...
 *
 * This method returns a vector containing the `x` coordinates of all points in
 * the cloud.
 *
 ...
 *
 * This method returns a vector containing the `y` coordinates of all points in
 * the cloud.
 *
 ...
 *
 * This method assigns new values to the cloud points based on a given vector of
 * values. The size of the input vector must match the number of points in the
 * cloud.
 *
 ...
 * @param bbox  The bounding box that defines the mapping from the cloud points'
 *              coordinates to the array's coordinates.
 */
void set_values_from_array(const Array       &array,
                           const Vec4<float> &bbox = {0.f, 1.f, 0.f, 1.f});

/**
 * @brief Sets point values based on their distance to the bounding box border.
 *
...
/**
 * @brief Set the values of the cloud points based on the distance to the convex
 * hull of the cloud.
 *
 * This method assigns values to the cloud points based on their distance to the
 * convex hull of the cloud. The convex hull must be initialized using
 ...
 * be populated with the values of the points at their corresponding positions.
 *
 ...
 *                             coordinate system.
 * @param interpolation_method The method used for interpolation (e.g., nearest
 *                             neighbor, bilinear).
 ...
 * bit depth, and shape. The bounding box determines the area of the data to be
 * included in the image, while the shape defines the dimensions of the output
 * image.
 *
 * @param fname The file name for the output PNG image. This should include the
 *              file extension (e.g., "output.png").
 ...
 *              included in the image. It is given as {xmin, xmax, ymin, ymax}.
 *              The default is {0.f, 1.f, 0.f, 1.f}.
 * @param depth An integer specifying the bit depth of the image. It should be a
 *              value defined by OpenCV (e.g., `CV_8U` for 8-bit unsigned). The
 *              default is `CV_8U`.
 * @param shape A `Vec2<int>` specifying the dimensions of the output image. It
 *              is given as {width, height}. The default is {512, 512}.
 */
