 ...
 * Initializes a path with a specified number of points, random values, and the
...
/**
 * @brief Construct a new Path object based on `x`, `y` coordinates, and values.
 * Initializes a path with the specified `x` and `y` coordinates, associated
 * values, and an option to be open or closed.
 * @param x      List of `x` coordinates for the points.
 * @param y      List of `y` coordinates for the points.
 ...
 *
 * This method removes all points and associated data from the path, effectively
 ...
 * values can represent any attribute associated with the points, such as color,
 ...
 * vector, where the coordinates are interleaved: `(x0, y0, x1, y1, ...)`. This
 * format is useful for operations that require a flat representation of the
 * coordinates.
 *
 ...
 * parameter.
 *
 * @param decreasing If true, enforces a monotonically decreasing order for the
 *                   values. If false, enforces a monotonically increasing order
 *                   for the values.
 *
 ...
 *
 * This method reorders the points in the path to minimize the total distance by
 * performing a nearest neighbor search starting from the specified
 ...
 *
 * This method reverses the sequence of points in the path, which can be useful
 * for various applications, such as changing the direction of the path
 * traversal.
 */
void reverse();
...
/**
 * @brief Return the angle of the closest edge to the point (x, y), assuming an
 * open path.
 *
 * This method calculates the angle of the edge closest to the specified point
 * (x, y) on a path that is open. The angle is returned in radians. The path is
 * assumed to have distinct start and end points, and the closest edge is
...
/**
 * @brief Return the signed distance function value at (x, y), assuming a closed
 * path.
 *
 * This method computes the signed distance from the point (x, y) to the nearest
 * edge of a closed path. The distance is signed, meaning it indicates whether
 * the point is inside or outside the path, with negative values typically
 * indicating that the point is inside the path.
 *
 * **Example**
 * @include ex_path_sdf.cpp
 *
 ...
 * distance from the closed path, adjusted by a downslope factor. The downslope
 * determines how quickly the elevation decreases as you move away from the
 * path.
 *
 * **Example**
 * @include ex_path_sdf.cpp
 ...
 *
 * This method calculates the elevation value at a point (x, y) considering its
 * distance from an open path and adjusting it based on a downslope factor. The
 * downslope determines how the elevation decreases as you move away from the
 * path.
 *
 * **Example**
 * @include ex_path_sdf.cpp
 ...
 *
 * This method computes the signed distance from the point (x, y) to the nearest
 * edge of an open path. The signed distance indicates how far the point is from
 * the path, with positive values typically representing the outside of the path
 * and negative values indicating the inside.
 *
 * **Example**
 * @include ex_path_sdf.cpp
 *
 ...
 *
 * This method reduces the number of points in the path by retaining only every
 * 'step'-th point, effectively subsampling the path. This can be useful for
 * simplifying the path or reducing data size.
 *
 ...
 * array based on the path's points. Optionally, the contour of the path can be
 * filled using flood fill if the `filled` parameter is set to true.
 *
 * **Example**
 ...
 * @param filled Boolean flag indicating whether to perform flood filling of the
 *               path's contour.
 */
 ...
 * distance from each point in the array to the nearest point on the path, with
 * positive values indicating distances outside the path and negative values
 * indicating distances inside the path.
 *
 * **Example**
 * @include ex_path_sdf.cpp
 *
 ...
 * @return            Array The resulting array filled with the signed distance
 *                    function values.
 */
