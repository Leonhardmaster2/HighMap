 ...
 *
 * This member variable stores the edges of the graph. Each edge is represented
 * as a pair of indices referring to the vertices in the graph. The edges are
 * stored as a vector of vectors, where each inner vector contains the indices
 * of vertices connected by that edge.
 */
 ...
 * weights are used to represent the cost or distance associated with traveling
 ...
 *
 * This member variable stores the connectivity information of the points in the
 * graph. It is represented as a vector of vectors, where each inner vector
 * contains the indices of neighboring vertices connected to the corresponding
 * point.
 */
 ...
 * map where each key is a pair of vertex indices and the value is the weight of
 * the edge connecting those vertices. The adjacency matrix provides a way to
 * quickly access the weight of an edge between any two vertices.
 */
 ...
 * `Cloud` object provides the points which will be used to construct the graph.
 * This constructor is useful when you have a `Cloud` object and want to create
 * a `Graph` representation from it.
 *
 ...
 * objects. The `Point` objects are used to populate the vertices of the graph.
 * This constructor is useful when you have a list of `Point` objects and want
 * to create a `Graph` representation from them.
 *
 ...
 * and y coordinates. The points are created from these coordinates and used to
 * populate the vertices of the graph. This constructor is useful when you have
 * x and y coordinates and want to create a `Graph` representation from them.
 *
 ...
 * the edge can be provided explicitly; if not provided, the Euclidean distance
 * between the connected points is used as the default weight.
 *
 * @param edge   A vector of two integers representing the indices of the points
 *               connected by the edge.
 * @param weight The weight of the edge. If not provided, the default weight is
 ...
 * target point in the graph using Dijkstra's algorithm. It returns a vector of
 * point indices representing the route from the source to the target point.
 *
 ...
 *                            representing the shortest path from the source to
 *                            the target.
 *
 * **Example**
 ...
 *
 * This method calculates the Euclidean length of a specific edge in the graph.
 * The edge is identified by the index `k`, and its length is determined by the
 * distance between the two vertices connected by the edge.
 *
 ...
 * graph. The coordinates are returned as a vector of floats, where each pair of
 * floats represents the x coordinates of an edge's endpoints.
 *
 ...
 * graph. The coordinates are returned as a vector of floats, where each pair of
 * floats represents the y coordinates of an edge's endpoints.
 *
 ...
 * are computed using the Euclidean distance formula, and the result is returned
 ...
 *
 * This method returns the total number of edges present in the graph. The edges
 * are stored in a vector, and this method returns the size of that vector,
 * which represents the number of edges.
 *
 ...
 * fractalization includes multiple iterations and random Gaussian displacement
 * to generate a fractal effect. The parameters control the number of
 * iterations, randomness, and how the graph path is altered.
 *
 ...
 * @param persistence Noise persistence factor with respect to iteration number.
 */
 ...
 *
 * This method exports a visual representation of the graph as a PNG image file.
 * The resolution of the image can be specified using the `shape`
 * parameter.
 *
 ...
 *
 * This method updates the adjacency matrix based on the current graph edges and
 * weights. The adjacency matrix represents the connectivity between nodes in
 * the graph.
 */
