...
/**
 * @brief Overloads the subtraction-assignment operator for scalar subtraction.
 *
...
/**
 * @brief Overloads the multiplication operator for element-wise multiplication
 * with another array.
 *
...
/**
 * @brief Overloads the addition operator for element-wise addition with another
 * array.
 *
...
/**
 * @brief Distribute a value 'amount' around the four cells (i, j), (i + 1, j),
 * (i, j + 1), (i + 1, j + 1) by "reversing" the bilinear interpolation.
 *
 * @param i      Index.
 * @param j      Index.
...
/**
 * @brief Distributes a specified amount of value around the cell located at (i,
 * j) using a 1D deposition kernel applied in both horizontal and vertical
 * directions.
 *
 ...
 *        from the original array, creating a new array. Note that i2 and j2 are
 * excluded from the slice.
 *
 * @param  idx A Vec4<int> containing the slice extent indices {i1, i2, j1, j2}.
 ...
 *
 *        This function computes the gradient in the x-direction, which measures
 * the rate of change along the x-axis. Note that this function cannot be used
 * at the borders of the data, as the central difference scheme requires values
 * from both sides of the point of interest.
 *
 ...
 * index (i, j) using a 2nd order central difference scheme.
 *
 *        This function computes the gradient in the y-direction, which measures
 * the rate of change along the y-axis. Note that this function cannot be used
 * at the borders of the data, as the central difference scheme requires values
 * from both sides of the point of interest.
 *
 * @warning The gradient cannot be computed at the borders of the data array.
 ...
 *
 *        This function uses bilinear interpolation to estimate the gradient in
 * the x-direction at a point within the cell defined by (i, j). The gradient is
 * based on a 2nd order central difference scheme but interpolated to a finer
 * resolution.
 *
 * @warning The gradient cannot be computed at the borders of the data array.
 * Specifically, the function is not defined for i = 0, j = 0, i =
 ...
 *
 *        This function uses bilinear interpolation to estimate the gradient in
 * the y-direction at a point within the cell defined by (i, j). The gradient is
 * based on a 2nd order central difference scheme but interpolated to a finer
 * resolution.
 *
 * @warning The gradient cannot be computed at the borders of the data array.
 ...
 * j) in the data array. The normal vector is a three-dimensional vector (x, y,
 * z) that is perpendicular to the surface at that point.
 *
...
/**
 * @brief Retrieves the array value at the location (x, y) near the index (i, j)
 ...
 * smoother result compared to bilinear interpolation by taking into account the
 * values of the surrounding 4x4 grid of points, resulting in better accuracy
 * when interpolating in areas with gradients or curves.
 *
 * @warning This function cannot be used at the upper borders of the array,
 * specifically for i = shape.x - 2 or j = shape.y - 2, as bicubic interpolation
 * requires surrounding data points from a 4x4 neighborhood.
 *
 * @param  i Index along the x-direction, expected to be in the range [1,
 *           shape.x - 3].
 * @param  j Index along the y-direction, expected to be in the range [1,
 ...
 * based on the surrounding values to approximate the value at the specified (x,
 * y) location.
 *
 * @warning This function cannot be used at the upper borders of the array,
 ...
 * coordinates from the array, assuming the array covers a domain defined by the
 * bounding box `bbox`. If the coordinates (x, y) are outside the domain, they
 * are clamped to the nearest valid position within the bounding box.
 *
 * @param  x    Coordinate along the x-axis.
 * @param  y    Coordinate along the y-axis.
 * @param  bbox Bounding box defining the domain of the array. It is expected to
 *              be in the form of {xmin, xmax, ymin, ymax}.
 ...
 *
 *        This function outputs details about the array, such as its dimensions,
 * data type, and any other relevant information. The optional
 ...
 * array.
 *
 *        This function calculates the linear index for a cell specified by the
 * (i, j) coordinates in a 2D array. The linear index is useful for accessing
 * elements in a flattened or 1D representation of the array.
 *
 ...
 * from a given linear index `k`. This is useful for mapping between the 1D and
 * 2D representations of the array.
 *
 ...
 *
 *        This function computes the maximum value present in the array. If the
 * array is empty, the behavior of this function is undefined.
 *
 ...
 *
 *        This function computes the minimum value present in the array. If the
 * array is empty, the behavior of this function is undefined.
 *
...
/**
 * @brief Normalize array values so that the sum of all elements is equal to 1.
 *
 *        This function adjusts the values in the array so that their total sum
 * is normalized to 1. This is often used in scenarios where the array
 ...
 *
 *        This function calculates the coefficients used to normalize the array
 * values so that they fit within the specified range [vmin, vmax]. The
 ...
 * format to the standard output. This can be useful for debugging or inspecting
 ...
 * returns it as a `std::vector<float>`. This is useful for operations where you
 * need to work with individual rows outside of the array structure.
 *
...
/**
 * @brief Set the value of a slice defined by indices {i1, i2, j1, j2} to a new
 * value.
 *
 *        This function updates all elements within the specified slice of the
 * array to the given `value`. The slice is defined by the extents {i1, i2, j1,
 * j2}, where `i1` and `i2` specify the range for the rows, and `j1` and
 ...
 *
 *        This function returns the size of the array, which is the total number
 * of elements it contains. This can be useful for iterating over the array or
 ...
 * to the `Array` will directly affect the corresponding
 ...
 * is a high dynamic range (HDR) image file format. The file will be created or
 * overwritten with the name specified by `fname`.
 *
 ...
 *
 *        This function writes the contents of the array to a file in raw binary
 * format. This can be useful for saving the array data to disk or for
 ...
 * hillshading.
 *
 *        This function saves the array data as a PNG image file. The colors of
 * the image are determined by the specified colormap (`cmap`), and optional
 * hillshading can be applied to enhance the visual representation of the data.
